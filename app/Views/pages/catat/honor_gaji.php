...
<div class="space-y-3">
    ...
    <form method="post" action="<?= site_url('catat/keluar/honor-gaji') ?>" enctype="multipart/form-data" class="space-y-4">
        ...
        <?= view('partials/capture_assist', [
            ...
            'description' => 'Nanti AI akan membaca bukti dokumen lalu membantu mengisi nominal, biaya admin, rekening, tanggal, dan keterangan.',
            'previewTitle' => 'Belum ada dokumen dipilih',
            'previewDescription' => 'Gunakan kamera untuk memfoto bukti fisik, atau upload gambar slip gaji dan bukti transfer jika ada.',
            'autoFields' => ['Nominal', 'Biaya Admin', 'Rekening', 'Tanggal', 'Keterangan'],
        ...
        <div class="rounded-3xl bg-white border border-zinc-100 p-6 text-center shadow-sm">
            <p class="text-xs font-semibold uppercase tracking-wider text-zinc-400">Total Dibayarkan (THP)</p>
        ...
        <div class="space-y-2">
            <label class="text-sm font-semibold text-zinc-900">Kategori</label>
        ...
        <div class="space-y-2">
            <label class="text-sm font-semibold text-zinc-900">Pilih Penerima / Karyawan</label>
            <div class="relative">
                <select name="receiver_id" class="h-12 w-full appearance-none rounded-2xl border border-zinc-100 bg-white pl-4 pr-10 text-sm text-zinc-950 outline-none transition focus:border-zinc-300 focus:ring-2 focus:ring-lime-400" required>
                    <option value="">-- Pilih dari Master Kontak --</option>
                    <?php foreach ($receivers as $receiver): ?>
                        <option value="<?= esc($receiver['id']) ?>" <?= (string) old('receiver_id', '') === (string) $receiver['id'] ? 'selected' : '' ?>><?= esc($receiver['name']) ?> (<?= esc($receiver['type'] ?? 'Lainnya') ?>)</option>
                    <?php endforeach; ?>
                </select>
            ...
            <p class="text-xs text-zinc-500">Penerima belum ada? <a href="<?= site_url('pengaturan/penerima/tambah') ?>" class="text-blue-600 font-medium hover:underline">Tambah baru</a>.</p>
        </div>
        ...
        <div class="space-y-2">
            <label class="text-sm font-semibold text-zinc-900">Keterangan / Periode</label>
            <textarea name="notes" rows="3" class="w-full rounded-2xl border border-zinc-100 bg-white px-4 py-3 text-sm text-zinc-950 outline-none transition focus:border-zinc-300 focus:ring-2 focus:ring-lime-400" placeholder="Misal: Gaji bulan Mei 2026 atau Honor narasumber seminar..." required><?= esc(old('notes', '')) ?></textarea>
        </div>
