...
<div class="space-y-3">
    ...
    <form action="<?= esc(site_url('catat/keluar/biaya')) ?>" method="post" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
                <?php foreach ($expenseCategories as $category): ?>
                    <label class="cursor-pointer">
                        ...
                        <span class="inline-block rounded-full px-3 py-2 text-sm font-medium bg-zinc-100 text-zinc-700 peer-checked:bg-lime-400 peer-checked:text-zinc-950 transition hover:bg-zinc-200">
                            <?= esc($category['name']) ?>
                        </span>
                    </label>
                <?php endforeach; ?>
        ...
        <?= view('partials/project_pocket_field', [
            ...
            'placeholder' => 'Pilih kantong biaya',
        ]) ?>
        ...
        <div class="space-y-2">
            ...
            <textarea name="notes" rows="3" placeholder="Contoh: Beli alat tulis" class="w-full rounded-2xl border border-zinc-100 bg-white px-4 py-3 text-sm text-zinc-950 outline-none transition focus:border-zinc-300 focus:ring-2 focus:ring-lime-400"><?= esc(old('notes', '')) ?></textarea>
        </div>
